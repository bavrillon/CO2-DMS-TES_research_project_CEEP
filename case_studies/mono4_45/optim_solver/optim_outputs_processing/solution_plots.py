...
def plot_load_real_distribution(day_type=['Low', 'Medium', 'High', 'Worst_day'], csv_path=OUTPUT_CSV_PATH, saving_path=None, show=False):
    ...
    for idx, dt in enumerate(selected_day_types):
        ...
        if idx % 2 == 0:
            ax.set_ylabel('$\dot{Q}$ (kW)', fontsize=label_size)
        else:
            ax.set_ylabel('')
        if dt == 'Worst_day':
            dt_name = 'Worst\nday'
            ...
            dt_name = dt
        ax.text(0.02, 0.95, dt_name, transform=ax.transAxes, ha='left', va='top',
        ...
        if legend_handles is None:
            ...
            legend_handles = [label_map[l] for l in order if l in label_map]
            legend_labels = [l for l in order if l in label_map]
    ...
    plt.tight_layout(pad=3.0, rect=(0, 0.21, 1, 1))

    if saving_path is not None:
        filename = 'solution_load_distribution.png'
        filepath = os.path.join(saving_path, filename)
        extra_artists = [legend_obj] if legend_obj is not None else None
        plt.savefig(filepath, dpi=300, bbox_inches='tight', bbox_extra_artists=extra_artists)
        print(f"Figure saved: {filepath}")
